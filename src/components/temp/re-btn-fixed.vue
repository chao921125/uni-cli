<template>
    <view class="re-btn-fixed" :style="styleObj">
        <view v-if="props.position === 'top'" class="re-w-fill nav-status"></view>
        <view class="re-w-fill">
            <button class="re-w-fill btn-primary" @tap="changeClick">
                <slot>{{ props.text }}</slot>
            </button>
        </view>
        <view v-if="props.position === 'bottom'" class="re-w-fill safe-area-bottom"></view>
    </view>
</template>
