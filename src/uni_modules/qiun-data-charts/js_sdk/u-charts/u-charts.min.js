...
function getMixToolTipData(t, e, a, i) {
	...
	var n = t.map(function (t) {
		return { text: r.formatter ? r.formatter(t, i[a], a, e) : t.name + ": " + t.data, color: t.color, disableLegend: t.disableLegend ? true : false };
	});
...
function calLegendData(l, s, e, t, h) {
	...
	let f = s.legend.margin * s.pix;
	let x = s.legend.fontSize ? s.legend.fontSize * s.pix : e.fontSize;
	let p = 15 * s.pix;
	let u = 5 * s.pix;
	let g = Math.max(s.legend.lineHeight * s.pix, x);
	if (s.legend.position == "top" || s.legend.position == "bottom") {
		...
		for (let a = 0; a < l.length; a++) {
			let t = l[a];
			let e = p + u + measureText(t.name || "undefined", x, h) + s.legend.itemGap * s.pix;
		...
		let t = l.length;
		let e = s.height - s.area[0] - s.area[2] - 2 * f - 2 * d;
		...
		if (r.length) {
			for (let t = 0; t < r.length; t++) {
				...
				let i = 0;
				for (let e = 0; e < a.length; e++) {
					let t = p + u + measureText(a[e].name || "undefined", x, h) + s.legend.itemGap * s.pix;
					if (t > i) {
						i = t;
					}
				}
...
function calCategoriesData(t, a, e, i, r) {
	...
	var n = a.xAxis.fontSize * a.pix || e.fontSize;
	var l = t.map(function (t) {
		var e = a.xAxis.formatter ? a.xAxis.formatter(t) : t;
...
function getCandleDataPoints(t, l, s, h, c, d, a) {
	var f = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : 1;
	var e = [];
	var x = d.height - d.area[0] - d.area[2];
...
function getYAxisTextList(t, e, h, c, a) {
	...
	var p = a.max === undefined || a.min === null ? l.maxRange : a.max;
	var g = [];
	var u = (p - x) / e.yAxis.splitNumber;
...
function calYAxisData(a, r, o, n) {
	var l = assign({}, { type: "" }, r.extra.column);
	var t = r.yAxis.data.length;
	var s = new Array(t);
	if (t > 0) {
		for (let e = 0; e < t; e++) {
			s[e] = [];
		...
		for (let i = 0; i < t; i++) {
			...
			if (e.type === "categories") {
				...
				if (!e.formatter) {
					e.formatter = (t) => {
						return t.toFixed(e.tofix) + (e.unit || "");
					};
				}
				h[i] = getYAxisTextList(s[i], r, o, l.type, e, i);
			}
			let a = e.fontSize * r.pix || o.fontSize;
		...
		if (r.type === "bar") {
			...
			if (!r.yAxis.formatter) {
				r.yAxis.formatter = (t) => {
					return t + (r.yAxis.unit || "");
				};
			}
		} else {
			if (!r.yAxis.formatter) {
				r.yAxis.formatter = (t) => {
					return t.toFixed(r.yAxis.tofix) + (r.yAxis.unit || "");
				};
			}
			h[0] = getYAxisTextList(a, r, o, l.type, {});
		}
		d[0] = { position: "left", width: 0 };
		var e = r.yAxis.fontSize * r.pix || o.fontSize;
		...
		if (r.yAxis.disabled === true) {
			...
			r.yAxis.data[0] = { disabled: false, position: "left", max: r.yAxis.max, min: r.yAxis.min, formatter: r.yAxis.formatter };
			if (r.type === "bar") {
...
function drawGaugeLabel(e, a, i, r, o, n) {
	...
	let f = e.endNumber - e.startNumber;
	let x = f / e.splitLine.splitNumber;
	let l = e.startAngle;
	let s = e.startNumber;
...
function drawRadarLabel(t, n, l, s, h, c) {
	...
	t.forEach(function (t, e) {
		var a = { x: n * Math.cos(t), y: n * Math.sin(t) };
		var i = convertCoordinateOrigin(a.x, a.y, l);
		var r = i.x;
		var o = i.y;
...
function drawPieText(n, d, f, x, t, l) {
	...
	var y = n.map(function (t, e) {
		...
		var r = t.color;
		var o = t._radius_;
		return { arc: i, text: a, color: r, radius: o, textColor: t.textColor, textSize: t.textSize };
	});
	for (let c = 0; c < y.length; c++) {
		...
		let n = a;
		let l = measureText(t.text, t.textSize * d.pix || f.fontSize, x);
		let s = n;
...
function drawMarkLine(s, h, c) {
	...
	for (let t = 0; t < r.length; t++) {
		...
		c.setLineDash([]);
		if (l.showLabel) {
			let t = l.labelText ? l.labelText : s.yAxis.formatter ? s.yAxis.formatter(l.value) : l.value;
			c.setFontSize(h.fontSize);
...
function drawToolTipHorizentalLine(d, f, x, t, e) {
	...
	x.setLineDash([]);
	if (p.yAxisLabel) {
		let l = calTooltipYAxisData(d.tooltip.offset.y, d.series, d, f, t);
		let s = d.chartData.yAxisData.yAxisWidth;
		let h = d.area[3];
		let c = d.width - d.area[1];
		for (let n = 0; n < l.length; n++) {
			x.setFontSize(f.fontSize);
			let t = measureText(l[n], f.fontSize, x);
			let e, a, i;
			...
			x.beginPath();
			x.setFillStyle(hexToRgb(p.labelBgColor || f.toolTipBackground, p.labelBgOpacity || f.toolTipOpacity));
...
function drawToolTipSplitArea(t, e, a, i, r) {
	var o = assign({}, { activeBgColor: "#000000", activeBgOpacity: 0.08 }, e.extra.column);
	var n = e.area[0];
...
function drawToolTip(t, r, e, o, n, c, d) {
	...
	var p = false;
	if (e.type == "line" || e.type == "mount" || e.type == "area" || e.type == "candle" || e.type == "mix") {
		if (l.splitLine == true) {
	...
	var h = f + x + 4 * o.toolTipPadding + Math.max.apply(null, g);
	...
	t.forEach(function (t, e) {
		if (t.color !== null) {
			...
			var i = r.y + (o.toolTipLineHeight - o.fontSize) / 2 + o.toolTipLineHeight * e + o.toolTipPadding + 1;
			if (p) {
	...
	t.forEach(function (t, e) {
		var a = r.x + s + 2 * o.toolTipPadding + f + x;
		if (p) {
...
function drawColumnDataPoints(v, m, T, b) {
	let w = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
	let t = m.chartData.xAxisData,
		P = t.xAxisPoints,
		A = t.eachSpacing;
	let S = assign(
		{},
			...
			case "group":
				...
				for (let t = 0; t < r.length; t++) {
					...
					if (o !== null && t > C && t < D) {
						...
						var p = o.color || a.color;
						if (S.linearType !== "none") {
							var e = b.createLinearGradient(n, o.y, n, m.height - m.area[2]);
							if (S.linearType == "opacity") {
								...
								e.addColorStop(S.colorStop, hexToRgb(S.customColor[a.linearIndex], S.linearOpacity));
								e.addColorStop(1, hexToRgb(s, 1));
							}
							s = e;
						}
...
function drawMountDataPoints(i, n, o, l) {
	let x = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
				...
				if (o !== null && t > a && t < g) {
					...
					if (s.linearType !== "none") {
						...
						if (s.linearType == "opacity") {
							...
							f.addColorStop(s.colorStop, hexToRgb(s.customColor[i[t].linearIndex], s.linearOpacity));
							f.addColorStop(1, hexToRgb(d, 1));
						}
						d = f;
					}
				...
				if (t !== null && e > a && e < g) {
					...
					if (s.linearType !== "none") {
						...
						if (s.linearType == "opacity") {
							...
							f.addColorStop(s.colorStop, hexToRgb(s.customColor[i[e].linearIndex], s.linearOpacity));
							f.addColorStop(1, hexToRgb(d, 1));
						}
						d = f;
					}
		...
		case "mount":
			for (let e = 0; e < h.length; e++) {
				...
				if (t !== null && e > a && e < g) {
					...
					l.moveTo(c, n.height - n.area[2]);
					l.bezierCurveTo(t.x - t.width / 4, n.height - n.area[2], t.x - t.width / 4, t.y, t.x, t.y);
					l.bezierCurveTo(t.x + t.width / 4, t.y, t.x + t.width / 4, n.height - n.area[2], c + t.width, n.height - n.area[2]);
					l.setStrokeStyle(T);
					l.setFillStyle(d);
					if (s.borderWidth > 0) {
						l.setLineWidth(s.borderWidth * n.pix);
		...
		case "sharp":
			for (let e = 0; e < h.length; e++) {
				...
				if (t !== null && e > a && e < g) {
					...
					l.quadraticCurveTo(t.x + 0, n.height - n.area[2] - m / 4, c + t.width, n.height - n.area[2]);
					l.setStrokeStyle(T);
					l.setFillStyle(d);
					if (s.borderWidth > 0) {
						l.setLineWidth(s.borderWidth * n.pix);
...
function drawBarDataPoints(y, v, m, T) {
	...
	let w = [];
	let P = (v.height - v.area[0] - v.area[2]) / v.categories.length;
...
function drawCandleDataPoints(e, t, h, c, d) {
	...
	if (s.average.show || t) {
		t.forEach(function (e, t) {
			...
			r = a.shift();
			var o = e.data;
			var n = getDataPoints(o, i, r, f, x, h, c, u);
	...
	e.forEach(function (t, e) {
		...
		r = a.shift();
		var o = t.data;
		var n = getCandleDataPoints(o, i, r, f, x, h, c, u);
		y.push(n);
...
function drawAreaDataPoints(t, s, h, c) {
	...
	var f = assign({}, { type: "straight", opacity: 0.2, addLine: false, width: 2, gradient: false }, s.extra.area);
	...
	t.forEach(function (e, t) {
		...
		r = a.shift();
		let o = e.data;
		let n = getDataPoints(o, i, r, y, x, s, h, d);
		v.push(n);
	...
	if (s.dataLabel !== false && d === 1) {
		t.forEach(function (t, e) {
			...
			r = a.shift();
			var o = t.data;
			var n = getDataPoints(o, i, r, y, x, s, h, d);
...
function drawScatterDataPoints(t, s, h, c) {
	...
	t.forEach(function (t, e) {
		...
		r = a.shift();
		var o = t.data;
		var n = getDataPoints(o, i, r, f, x, s, h, d);
		c.beginPath();
...
function drawBubbleDataPoints(l, s, h, c) {
	var d = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
	var f = assign({}, { opacity: 1, border: 2 }, s.extra.bubble);
	...
	l.forEach(function (t, e) {
		...
		r = a.shift();
		var o = t.data;
		var n = getDataPoints(o, i, r, x, p, s, h, d);
		c.beginPath();
		c.setStrokeStyle(t.color);
...
function drawLineDataPoints(t, s, h, c) {
	var d = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
	var f = assign({}, { type: "straight", width: 2 }, s.extra.line);
	f.width *= s.pix;
	let e = s.chartData.xAxisData,
		x = e.xAxisPoints,
		p = e.eachSpacing;
...
function drawMixDataPoints(t, v, m, T) {
	...
	let P = assign(
		{},
		{ width: w / 2, barBorderCircle: false, barBorderRadius: [], seriesGap: 2, linearType: "none", linearOpacity: 1, customColor: [], colorStop: 0 },
		v.extra.mix.column,
	);
	let A = v.height - v.area[2];
	let M = [];
	var L = 0;
	var _ = 0;
	t.forEach(function (t, e) {
	...
	t.forEach(function (n, t) {
		...
		x = o.shift();
		var p = n.data;
		var a = getDataPoints(p, f, x, b, w, v, m, D);
		M.push(a);
		if (n.type == "column") {
	...
	if (v.dataLabel !== false && D === 1) {
		...
		t.forEach(function (t, e) {
			...
			r = a.shift();
			var o = t.data;
			var n = getDataPoints(o, i, r, b, w, v, m, D);
			if (t.type !== "column") {
...
function drawToolTipBridge(t, e, a, i, r, o) {
	...
	if (
		...
		t.tooltip &&
		i === 1 &&
		(t.type == "line" || t.type == "area" || t.type == "column" || t.type == "mount" || t.type == "candle" || t.type == "mix")
	) {
...
function drawXAxis(r, l, s, h) {
	let t = l.chartData.xAxisData,
		...
		d = t.eachSpacing;
	var p = "center";
	if (l.type == "bar" || l.type == "line" || l.type == "area" || l.type == "scatter" || l.type == "bubble") {
		p = l.xAxis.boundaryGap;
	}
	var f = l.height - l.area[2];
	var i = l.area[0];
	if (l.enableScroll && l.xAxis.scrollShow) {
		var o = l.height - l.area[2] + s.xAxisHeight;
		var n = a - e;
		var g = d * (c.length - 1);
		if (l.type == "mount" && l.extra && l.extra.mount && l.extra.mount.widthRatio && l.extra.mount.widthRatio > 1) {
	...
	if (l.xAxis.disabled !== true) {
		...
		if (s._xAxisTextAngle_ === 0) {
			...
			a.forEach(function (t, e) {
				...
				h.setFontSize(x);
				h.setFillStyle(l.xAxis.fontColor || l.fontColor);
				var i = measureText(String(a), x, h);
				var r = c[e];
				if (p == "center") {
					r = c[e] + d / 2;
				}
				...
				var n = f + 6 * l.pix + x - x * Math.abs(Math.sin(s._xAxisTextAngle_));
...
function drawYAxis(e, h, a, c) {
	...
	let x = h.area[3] + (h.width - h.area[1] - h.area[3]) / 2;
	if (h.yAxis.data) {
...
function drawLegend(t, n, u, l, e) {
	...
	let a = s.points;
	let h = s.area;
	let c = n.legend.padding * n.pix;
	let d = n.legend.fontSize * n.pix;
	let f = 15 * n.pix;
	let x = 5 * n.pix;
	let p = n.legend.itemGap * n.pix;
	let g = Math.max(n.legend.lineHeight * n.pix, d);
	l.beginPath();
...
function drawArcbarDataPoints(a, i, t, r) {
	...
	var o = assign({}, { startAngle: 0.75, endAngle: 0.25, type: "default", width: 12, gap: 2, linearType: "none", customColor: [] }, i.extra.arcbar);
	...
	for (let e = 0; e < a.length; e++) {
		...
		r.beginPath();
		if (o.type == "default") {
			r.arc(n.x, n.y, l - (o.width * i.pix + o.gap * i.pix) * e, o.startAngle * Math.PI, o.endAngle * Math.PI, false);
		} else {
		...
		r.setLineCap("round");
		r.beginPath();
		r.arc(n.x, n.y, l - (o.width * i.pix + o.gap * i.pix) * e, o.startAngle * Math.PI, t._proportion_ * Math.PI, false);
		r.stroke();
	}
...
function drawGaugeDataPoints(n, h, c, t, d) {
	var f = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 1;
	var x = assign(
		{},
		{
			type: "default",
	...
	if (x.type == "progress") {
		...
		let e = y / x.splitLine.splitNumber;
		let a = y / x.splitLine.splitNumber / x.splitLine.childNumber;
		let i = -g - x.width * 0.5 - x.splitLine.fixRadius;
		let r = -g - x.width - x.splitLine.fixRadius + x.splitLine.width;
		d.save();
		d.translate(p.x, p.y);
		d.rotate((x.startAngle - 1) * Math.PI);
		let o = x.splitLine.splitNumber * x.splitLine.childNumber + 1;
		let n = h[0].data * f;
		for (let t = 0; t < o; t++) {
			d.beginPath();
			if (n > t / o) {
		...
		let a = y / x.splitLine.splitNumber / x.splitLine.childNumber;
		let i = -g - x.width * 0.5 - x.splitLine.fixRadius;
		let r = -g - x.width * 0.5 - x.splitLine.fixRadius + x.splitLine.width;
		let o = -g - x.width * 0.5 - x.splitLine.fixRadius + x.splitLine.childWidth;
		d.translate(p.x, p.y);
		d.rotate((x.startAngle - 1) * Math.PI);
...
function drawRadarDataPoints(r, o, i, n) {
	...
	var l = assign({}, { gridColor: "#cccccc", gridType: "radar", opacity: 0.2, gridCount: 3, border: false, borderWidth: 2 }, o.extra.radar);
	var a = getRadarCoordinateSeries(o.categories.length);
	var s = { x: o.area[3] + (o.width - o.area[1] - o.area[3]) / 2, y: o.area[0] + (o.height - o.area[0] - o.area[2]) / 2 };
	var c = (o.width - o.area[1] - o.area[3]) / 2;
	var d = (o.height - o.area[0] - o.area[2]) / 2;
	var h = Math.min(c - (getMaxTextListLength(o.categories, i.fontSize, n) + i.radarLabelTextMargin), d - i.radarLabelTextMargin);
	...
	a.forEach(function (t) {
		var e = convertCoordinateOrigin(h * Math.cos(t), h * Math.sin(t), s);
		n.moveTo(s.x, s.y);
		n.lineTo(e.x, e.y);
	});
	n.stroke();
	...
	var f = function t(i) {
		...
		if (l.gridType == "radar") {
			a.forEach(function (t, e) {
				var a = convertCoordinateOrigin((h / l.gridCount) * i * Math.cos(t), (h / l.gridCount) * i * Math.sin(t), s);
				if (e === 0) {
					r = a;
					n.moveTo(a.x, a.y);
				} else {
			...
			var e = convertCoordinateOrigin((h / l.gridCount) * i * Math.cos(1.5), (h / l.gridCount) * i * Math.sin(1.5), s);
			n.arc(s.x, s.y, s.y - e.y, 0, 2 * Math.PI, false);
		}
		n.stroke();
		n.closePath();
	};
	for (var e = 1; e <= l.gridCount; e++) {
		f(e);
	}
	var x = getRadarDataPoints(a, s, h, r, o, t);
	...
	if (o.dataLabel !== false && t === 1) {
		x.forEach(function (t, e) {
			n.beginPath();
			var a = t.textSize * o.pix || i.fontSize;
			n.setFontSize(a);
...
function drawMapDataPoints(c, a, d, i) {
	...
	if (r.mercator) {
		var f = lonlat2mercator(l.xMax, l.yMax);
		var x = lonlat2mercator(l.xMin, l.yMin);
		l.xMax = f[0];
		l.yMax = f[1];
		l.xMin = x[0];
		l.yMin = x[1];
		...
		case "normal":
			for (let l = 0; l < f.length; l++) {
				...
				while (true) {
					...
					let t = collisionNew(o, f, c.width, c.height);
					if (!t) break;
					if (n == 1e3) {
						o = [-100, -100, -100, -100];
						break;
...
function drawFunnelDataPoints(e, a, t, i) {
	...
	let d = Math.min((a.width - a.area[1] - a.area[3]) / 2 - l, (a.height - a.area[0] - a.area[2]) / 2 - l);
...
function drawCharts(t, l, n, s) {
	...
	var a = calLegendData(f, l, n, l.chartData, s),
		...
		case "candle":
			this.animationInstance = new Animation({
				...
				onProcess: function t(e) {
					...
					drawYAxisGrid(d, l, n, s);
					drawXAxis(d, l, n, s);
					var a = drawCandleDataPoints(c, f, l, n, s, e),
						i = a.xAxisPoints,
						r = a.calPoints,
						o = a.eachSpacing;
					l.chartData.xAxisPoints = i;
...
uCharts.prototype.dobuleZoom = function (t) {
	...
	for (var e = 0; e < t.changedTouches.length; e++) {
		t.changedTouches[e].x = t.changedTouches[e].x ? t.changedTouches[e].x : t.changedTouches[e].clientX;
		t.changedTouches[e].y = t.changedTouches[e].y ? t.changedTouches[e].y : t.changedTouches[e].clientY;
	}
	const a = t.changedTouches;
	const c = Math.abs(a[0].x - a[1].x);
	if (!this.scrollOption.moveCount) {
		const s = this.getCurrentDataIndex({ changedTouches: [{ x: a[0].x, y: this.opts.area[0] }] }).index;
		const h = this.getCurrentDataIndex({ changedTouches: [{ x: a[1].x, y: this.opts.area[0] }] }).index;
	...
	let d = (c * this.opts.pix) / this.scrollOption.moveCount;
	...
	let o = getXAxisPoints(this.opts.categories, this.opts, this.config),
		...
		p = o.endX,
		n = o.eachSpacing;
	let g = n * this.scrollOption.moveCurrent1;
	let u = p - x;
	let l = u - n * (f.length - 1);
...
uCharts.prototype.showToolTip = function (t) {
		...
		if (o > -1 || o.length > 0) {
			...
			if (n.length !== 0) {
				var l = getToolTipData(n, this.opts, o, r.group, this.opts.categories, e),
					...
					h = l.offset;
				h.y = a.y;
				i.tooltip = { textList: e.textList !== undefined ? e.textList : s, offset: e.offset !== undefined ? e.offset : h, option: e, index: o };
			}
		}
	...
	if (this.opts.type === "mount") {
		...
		if (o > -1) {
			...
			var s = [{ text: e.formatter ? e.formatter(n, undefined, o, i) : n.name + ": " + n.data, color: n.color }];
			var h = { x: i.chartData.calPoints[o].x, y: a.y };
			i.tooltip = { textList: e.textList ? e.textList : s, offset: e.offset !== undefined ? e.offset : h, option: e, index: o };
		}
		drawCharts.call(this, i.type, i, this.config, this.context);
	}
	...
	if (this.opts.type === "mix") {
		...
		if (o > -1) {
			...
			if (n.length !== 0) {
				var f = getMixToolTipData(n, this.opts, o, this.opts.categories, e),
					s = f.textList,
					h = f.offset;
				h.y = a.y;
				i.tooltip = { textList: e.textList ? e.textList : s, offset: e.offset !== undefined ? e.offset : h, option: e, index: o };
	...
	if (this.opts.type === "map") {
		...
		if (o > -1) {
			...
			var s = [{ text: e.formatter ? e.formatter(n, undefined, o, this.opts) : n.name, color: n.color }];
			var h = { x: a.x, y: a.y };
			i.tooltip = { textList: e.textList ? e.textList : s, offset: e.offset !== undefined ? e.offset : h, option: e, index: o };
		}
		i.updateData = false;
	...
	if (this.opts.type === "radar") {
		...
		if (o > -1) {
			...
			if (n.length !== 0) {
				var s = n.map((t) => {
					return { text: e.formatter ? e.formatter(t, this.opts.categories[o], o, this.opts) : t.name + ": " + t.data, color: t.color };
				});
				var h = { x: a.x, y: a.y };
				i.tooltip = { textList: e.textList ? e.textList : s, offset: e.offset !== undefined ? e.offset : h, option: e, index: o };
			}
